const CORRECT_USERNAME = 'Janseva';
const CORRECT_PASSWORD = 'Janseva@123';
...
document.getElementById('login-page').addEventListener('keydown', (event) => {
    if (event.key === 'Enter') {
        handleLogin();
    }
});
...
function recalc() {
    ...
    document.querySelectorAll('#servicesBody tr').forEach(tr => {
        ...
        const amt = qty * price;
        tr.querySelector('.amt').textContent = currency(amt);
        srv += amt;
    });
...
function resetForm() {
    ['patientName', 'age', 'gender', 'mobile', 'email', 'address', 'bp', 'pulse', 'temp', 'spo2', 'height', 'weight', 'rbs', 'date'].forEach(id => {
        const element = document.getElementById(id);
        if (element) element.value = '';
    });
    document.getElementById('servicesBody').innerHTML = '';
    document.getElementById('medsBody').innerHTML = '';
    addServiceRow('Registration Charges');
    addServiceRow('Room Rent');
    addServiceRow('Consultant Charges');
    addServiceRow('OT Charges');
    addMedicineRow();
    recalc();
    checkSectionVisibility();
}
